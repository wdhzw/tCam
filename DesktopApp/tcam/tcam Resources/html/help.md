...
* Discover - Displays the Camera Discovery window which is used to display the IP addresses of cameras on the same network (running current firmware).
...
* Stream - Start (or Stop) the camera streaming images at the rate set in the Preferences Window.  The stream may optionally be stored to a video file.  Clicking the Stream button brings up a submenu.
	...
	* Record - Select a video file to record the stream into and then display the stream.
...
2. Documentation - Displays application documentation in a window.
...
#### Version History
##### 3.1.0 - Bug fixes / Match tCam-Mini FW 3.1

1.  Support 63 character WiFi passwords with all tCam firmware and tCam-Mini firmware 3.1 and beyond.
2. Disabled Graph when connected to camera with Lepton 3.0.
3. Display "RAW" or "AGC" in color palette bar for Lepton 3.0.
4. Worked around Xojo bug where pull-down menus didn't work properly in Linux builds (starting with version 3.0.0).

##### 3.0.0 - Major update

1. Added new IsoTherm palette.
